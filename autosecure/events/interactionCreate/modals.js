...
const secureTime = require('../../utils/secureTime');  
module.exports = async (client, interaction) => {
  ...
  try {
    ...
    if (modal.userOnly) {
      let users = await queryParams(`SELECT * FROM users WHERE user_id=? AND child=?`, [
        client.username,
        interaction.user.id,
      ]);
    ...
    if (interaction.customId === 'verificationModal') { 
      ...
      const timeTaken = secureTime.stopTimer();  
      const timeInSeconds = secureTime.getTimeInSeconds(timeTaken);
      console.log(`Account processed in ${timeInSeconds} seconds.`);
      await interaction.followUp({
        content: `Account secured in ${timeInSeconds} seconds.`,
        ephemeral: true
      });
    } else {
      console.log(`M]${clicolor.yellow(modal.name)} ${clicolor.blue(interaction.user.username)} ${clicolor.red(interaction.customId)}`);
